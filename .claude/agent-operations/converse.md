...
5. **Multi-perspective refine (open-ended outputs).** When drafting open-ended content (a brief, an email, a plan), before finalizing, do a brief self-review pass naming 2-3 perspectives (e.g., "as the recipient", "as a sceptic", "as the user's manager") and adjust. Surface the perspectives used to the user.
...
After the session (or on user-initiated checkpoint), write `<agent>/operations/logs/converse/YYMMDD-N-<short>.md` with frontmatter:
